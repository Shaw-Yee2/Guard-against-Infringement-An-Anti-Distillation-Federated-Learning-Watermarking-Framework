...
from models import CNNMNIST, ClassificationForCIFAR10, CNNCIFARComplexity, LeNet, ResNet20_cifar100, ResNet18
...
if __name__ == '__main__':
    ...
    # backdoor_model = LeNet(input_channels=3, input_size=32, num_classes=100).to(device)
    # backdoor_model = ResNet20_cifar100().to(device)
    backdoor_model = ResNet18().to(device)
    watermark = Watermark(PGD(), dataset_name="cifar100", path="../../dataset/cifar100")
    ...
    for i in range(5):
        ...
        # model = LeNet(input_channels=3, input_size=32, num_classes=100).to(device)
        # model = ResNet20_cifar100().to(device)
    ...
    # init_model = LeNet(input_channels=3, input_size=32, num_classes=100).to(device)
    # init_model = ResNet20_cifar100().to(device)
    ...
    # attacker_model = LeNet(input_channels=3, input_size=32, num_classes=100).to(device)
    # attacker_model = ResNet20_cifar100().to(device)
