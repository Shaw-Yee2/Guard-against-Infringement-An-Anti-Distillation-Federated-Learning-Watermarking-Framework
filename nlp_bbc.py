...
if __name__ == '__main__':
    ...
    dataset = load_dataset("../../dataset/bbc_news")
    tokenizer = AutoTokenizer.from_pretrained("../../tokenizers/bert")


    def preprocess_data(_texts, _labels, _tokenizer, _max_length=128):
        encoding = _tokenizer(_texts, padding='max_length', truncation=True, max_length=_max_length, return_tensors='pt')
        input_ids = encoding['input_ids']
        attention_mask = encoding['attention_mask']
        _labels = torch.tensor(_labels)
        print(_labels)
        return TensorDataset(input_ids, _labels)
    ...
    tensor_dataset = preprocess_data(texts, labels, tokenizer)

    train_size = int(0.8 * len(tensor_dataset))
    test_size = len(tensor_dataset) - train_size
    data_train, data_test = random_split(tensor_dataset, [train_size, test_size])
    ...
    client_list = []
    for i in range(5):
        # model = TextCNN(tokenizer.vocab_size, num_classes=5).to(device)
        # model = LSTMClassifier(tokenizer.vocab_size).to(device)
        model = CBOWClassifier(tokenizer.vocab_size, num_classes=5).to(device)
    ...
    # init_model = TextCNN(tokenizer.vocab_size, num_classes=5).to(device)
    # init_model = LSTMClassifier(tokenizer.vocab_size).to(device)
    init_model = CBOWClassifier(tokenizer.vocab_size, num_classes=5).to(device)
    ...
    # attacker_model = TextCNN(tokenizer.vocab_size, num_classes=5).to(device)
    # attacker_model = LSTMClassifier(tokenizer.vocab_size).to(device)
    attacker_model = CBOWClassifier(tokenizer.vocab_size, num_classes=5).to(device)
